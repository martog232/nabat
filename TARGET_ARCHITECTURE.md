...
**Статус:** предложение. **Фаза 0 е завършена; Фаза 1 е изпълнена дотам, докъдето има смисъл** (виж раздел 8); **от Фаза 2 е свършен outbox-ът в nabat-voting**. Останалите фази не са започвани, но част от предпоставките им вече са налице. **Последно сверено с кода: 2026-08-27.**
...
| Event-driven комуникация между услугите | ❌ **Все още не в nabat-app.** `pom.xml` на nabat-app няма нито Kafka, нито Resilience4j — проверено. `kafka.yaml` в chart-а е за nabat-voting. Topic-ът `vote.cast` остава self-loop: nabat-voting сам публикува и сам консумира. Спечеленото дотук е pattern-ът, не транспортът — Event Publication Registry (V11) е transactional outbox в рамките на един процес, тоест Фаза 2 подменя носителя, а не идеята. **Промяна (2026-08-27):** от двете страни на този self-loop pattern-ът вече е приложен и през мрежа — nabat-voting има собствен outbox (`outbox_event` + `OutboxRelay`), така че `vote.cast` вече не е dual write. |
...
**Остава по фазата:** Kafka в самия nabat-app (`pom.xml` още няма нито Kafka, нито Resilience4j), Schema Registry с Avro и наложена съвместимост в CI, замяната на синхронния запис на броя гласове с consumer на `vote.cast`, и Resilience4j върху останалия синхронен hop.
