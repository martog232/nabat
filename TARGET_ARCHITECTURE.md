...
**Статус:** предложение. **Фаза 0 е завършена; Фаза 1 е изпълнена дотам, докъдето има смисъл** (виж раздел 8); **от Фаза 2 е свършен целият поток на гласовете — outbox в nabat-voting и consumer в nabat-app**. Останалите фази не са започвани, но част от предпоставките им вече са налице. **Последно сверено с кода: 2026-08-27.**
...
| Event-driven комуникация между услугите | 🟡 **Има я, за един поток (2026-08-27).** `vote.cast` и `vote.removed` вече не са self-loop: nabat-voting ги пише през transactional outbox (`outbox_event` + `OutboxRelay`), а nabat-app ги консумира и от тях поддържа денормализираните броячи върху `alerts`. Това е първият истински cross-service поток и покрива и двете страни на pattern-а — durable publish и идемпотентно прилагане. Останалото е непроменено: няма Schema Registry (JSON без контрактна проверка), няма Resilience4j никъде, а всички останали междумодулни събития са в процеса през Event Publication Registry (V11) и чакат съответната фаза. |
...
**Свършено (2026-08-27), втора част:** `vote.cast` вече не е self-loop. nabat-app има Kafka в `pom.xml` и consumer (`VoteEventListener` → `VoteTalliesProjectionService`), който пише денормализираните броячи върху `alerts`; `ExternalVoteService` вече не ги пише. Гласът остава синхронен, защото извикващият трябва да получи отговор, но **следствието** му вече не язди на този отговор — събитието се преиграва, докато не бъде приложено, вместо да се загуби при краш между отдалечения глас и локалния запис.
...
**Известна дупка, записана в AGENTS.md:** подредбата е гарантирана в рамките на topic, а гласът и оттеглянето му са в два. Бърз unvote може да се приложи преди гласа. Фиксът е watermark върху `alerts` — не е направен.
...
| Kafka като вътрешнопроцесен message bus | Сегашният self-loop на `vote.cast`: latency и цял outbox, за да е коректен, срещу нулево decoupling. |
