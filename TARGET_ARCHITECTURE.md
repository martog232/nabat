...
**Тестът вече съществува:** `VotingServiceIntegrationTest` вдига nabat-voting, Kafka и втори Postgres с Testcontainers и гласува по истинския HTTP път. На първото си пускане извади два реални дефекта — образът не можеше да мигрира схемата си (Boot 4 премести autoconfiguration-а на Flyway в отделен модул), а проекцията за достоверност **се състезава с commit-а**: `vote.cast` се публикува вътре в транзакцията, която записва гласа, а consumer-ът преизчислява от write model-а. Загуби ли надпреварата, записва нули — завинаги, защото преизчисляване има само при следващо събитие за същия alert. Наблюдавано и в двете посоки.

Това е мотивиращият пример за тази фаза. Outbox-ът не е упражнение за pattern заради самия pattern: без него проекцията е недетерминирана. Внимание при поправката — публикуване след commit стеснява прозореца, но не го затваря, защото изпращане след commit може да се загуби.
