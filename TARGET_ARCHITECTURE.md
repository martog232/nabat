...
**Статус:** предложение. **Фаза 0 е завършена; Фаза 1 е изпълнена дотам, докъдето има смисъл** (виж раздел 8); **от Фаза 2 е свършен целият поток на гласовете — outbox в nabat-voting и consumer в nabat-app**. Останалите фази не са започвани, но част от предпоставките им вече са налице. **Последно сверено с кода: 2026-08-31.**
...
| Събиране на метрики | ✅ **Поправено в двете среди.** И `prometheus.yml` (compose), и `prometheus-config.yaml` (Helm) задават `metrics_path: /actuator/prometheus` за двете услуги, а метриките на Kong се взимат от status listener-а на 8100, не от Admin API — двата пътя сервират `/metrics`, но само единият е и control surface, способен да замени цялата конфигурация на gateway-а. Target-ът `nabat-voting` вече се решава — услугата е в този стек зад профил `voting`. Без профила стои DOWN, което е вярно, а не неподредено: услугата наистина липсва и всеки глас връща 503. |
...
| Event-driven комуникация между услугите | 🟡 **Има я, за един поток (2026-08-31).** `vote.changed` вече не е self-loop: nabat-voting го пише през transactional outbox (`outbox_event` + `OutboxRelay`), а nabat-app го консумира и от него поддържа денормализираните броячи върху `alerts`. Това е първият истински cross-service поток и покрива трите страни на pattern-а — durable publish, идемпотентно прилагане и подредба по агрегат (един topic, ключ по alert). Останалото е непроменено: няма Schema Registry (JSON без контрактна проверка), няма Resilience4j никъде, а всички останали междумодулни събития са в процеса през Event Publication Registry (V11) и чакат съответната фаза. |
...
- **Замяната на синхронния запис на броя гласове с consumer на `vote.changed`** е описана в
...
| `vote.changed` | `incident_id` | voting | feed, notification, incident | Всяка промяна на гласовете — подаване, промяна на мнение, оттегляне — с `changeType` в payload-а. Ключът е по incident, за да останат всички промени за един incident подредени в един дял. **Един topic, не два (2026-08-31):** Kafka подрежда в рамките на дял, а два topic-а имат два набора дялове и два независими consumer container-а, така че бързо оттегляне можеше да се приложи преди самия глас. Consumer, който преизчислява от източника (nabat-voting), не забелязва; consumer, който прилага носените стойности (nabat-app), оставаше с грешни числа до следващия глас. |
...
Kafka в nabat-app. Schema Registry с Avro. Transactional outbox в двете услуги. **Замяна на синхронния запис на броя гласове с consumer на `vote.changed`** — self-loop-ът става истински cross-service поток. Resilience4j върху останалия синхронен hop.
...
**Тестът вече съществува:** `VotingServiceIntegrationTest` вдига nabat-voting, Kafka и втори Postgres с Testcontainers и гласува по истинския HTTP път. На първото си пускане извади два реални дефекта — образът не можеше да мигрира схемата си (Boot 4 премести autoconfiguration-а на Flyway в отделен модул), а проекцията за достоверност **се състезаваше с commit-а**: събитието за гласа се публикуваше вътре в транзакцията, която записва гласа, а consumer-ът преизчислява от write model-а. Загуби ли надпреварата, записваше нули — завинаги, защото преизчисляване има само при следващо събитие за същия alert. Наблюдавано и в двете посоки.

Това беше мотивиращият пример за тази фаза, и е **свършен (2026-08-27)**: nabat-voting пише събитието в `outbox_event` в транзакцията на гласа, а `OutboxRelay` праща само commit-нати редове (`FOR UPDATE SKIP LOCKED`; провален send спира партидата си, за да не се разбърка редът за един alert). Consumer-ът вече не може да преизчисли от write model без гласа в него. Доставката е at-least-once — send-ът и `published_at` също не са атомарни — което е безопасно, защото преизчисляването е идемпотентно. Недетерминизмът беше причината този тест само да *печата* броя от проекцията; сега `theProjectionEndsUpAgreeingWithTheWriteModel` го твърди.

Внимание, ако някой пипа това пак: публикуване след commit стеснява прозореца, но не го затваря, защото изпращане след commit може да се загуби. Само ред, който commit-ва заедно с гласа, маха въпроса.

**Свършено (2026-08-27), втора част:** топикът на гласовете вече не е self-loop. nabat-app има Kafka в `pom.xml` и consumer (`VoteEventListener` → `VoteTalliesProjectionService`), който пише денормализираните броячи върху `alerts`; `ExternalVoteService` вече не ги пише. Гласът остава синхронен, защото извикващият трябва да получи отговор, но **следствието** му вече не язди на този отговор — събитието се преиграва, докато не бъде приложено, вместо да се загуби при краш между отдалечения глас и локалния запис.

За да е възможно това, събитията носят `tallies` — абсолютни стойности, изчислени в транзакцията на гласа. Без тях consumer-ът има само два лоши избора: delta (двойно броене при at-least-once) или callback за stats (синхронен hop към асинхронна проекция). Абсолютните стойности правят прилагането идемпотентно, а `auto-offset-reset=earliest` — преиграване на topic-а, което възстановява цялата проекция.

**Свършено (2026-08-31), трета част — подредбата.** Дотук гласът и оттеглянето му бяха в два topic-а, а Kafka подрежда в рамките на дял: нищо не ги подреждаше един спрямо друг, и бърз unvote можеше да се приложи преди гласа. Двете вече са `vote.changed` с ключ по alert — един topic и един ключ значат един дял, а един дял значи ред. Дефектът не се засича и компенсира, а не може да се случи. Пинато от `aRetractionAfterACastLeavesTheRetractionsCounts`.

Алтернативата беше watermark в consumer-а: носим времето на събитието, пазим последно приложеното, игнорираме по-старите. Компенсира вместо да предотвратява, иска колона и миграция във **всеки** consumer с проекция, и струва колкото часовника на producer-а — тук по-малко, отколкото изглежда, защото `castAt` се взима преди четенето на броячите, тъй че при едновременни гласове по един alert по-новото четене може да носи по-старо време и да бъде отхвърлено като остаряло.

Домейнът запази двете събития. `VoteCastEvent` и `VoteRemovedEvent` са различни неща за казване; `VoteChangedMessage` е формата на транспорта, сглобена на границата на outbox-а.

**Остава по фазата:** Schema Registry с Avro и наложена съвместимост в CI, нотификациите като consumer (иска dedupe ключ, затова са във Фаза 6), и Resilience4j върху останалия синхронен hop.
...
| Kafka като вътрешнопроцесен message bus | Предишният self-loop на топика на гласовете: latency и цял outbox, за да е коректен, срещу нулево decoupling. |
