...
- **The vote counts on `alerts` are written by a consumer, not by the vote.** `VoteEventListener` (`voting/adapter/in/kafka`) reads `vote.cast` / `vote.removed` and drives `VoteTalliesProjectionService`, which calls `AlertRepository.applyVoteCounts` and broadcasts the update. The events carry the tallies as absolute values, so a redelivery is the same write; `spring.kafka.consumer.auto-offset-reset=earliest` is deliberate, since replaying the topic rebuilds the whole projection. Do not add a second write on the request path — the request would race the event for the same row.
...
| Kafka dual write | ✅ Done | nabat-voting writes `vote.cast` and `vote.removed` to an `outbox_event` row in the transaction that writes the vote, and `OutboxRelay` sends the committed rows (`SELECT ... FOR UPDATE SKIP LOCKED`, a failed send stops its batch to keep per-alert order). The consumer can no longer recompute the projection from a write model that does not hold the vote yet, which is what used to store zeros permanently. Delivery is at-least-once; recomputation is idempotent, so that is safe. `theProjectionEndsUpAgreeingWithTheWriteModel` in `VotingServiceIntegrationTest` asserts the count that the same test could only print before. An after-commit publish was the wrong fix and stays wrong: it narrows the window without closing it. |
| Vote counts written on the request path | ✅ Done | `VoteEventListener` consumes `vote.cast` / `vote.removed` and drives `VoteTalliesProjectionService`; `ExternalVoteService` no longer writes the counts. The events carry the tallies as absolute values, which is what makes applying them idempotent. Gated by `nabat.kafka.enabled`. |
| Vote events reordered across topics | 🟡 Known | Order is guaranteed within a topic, and a cast and its retraction are on two. A voter who unvotes immediately can have the removal applied before the cast, leaving the counts one vote high until the next event for that alert. nabat-voting's own consumer is immune only because it recomputes from the write model, which this one cannot — it does not own it. The fix is a watermark: carry the event timestamp, keep the last applied one on `alerts`, ignore anything older. |
