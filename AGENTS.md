...
- **Messages are Avro against a schema registry.** `src/main/avro/VoteChanged.avsc` is a verbatim copy of nabat-voting's, and the class is generated from it (module `events`). A message is a magic byte, a schema id and an Avro body, so `nabat.schema-registry.url` is required whenever the listener is on. The compatibility level is `BACKWARD_TRANSITIVE`, not `BACKWARD`, because this consumer replays the topic from the beginning: the oldest message ever written is one it will read again. The build-time gate lives in nabat-voting (`VoteChangedSchemaCompatibilityTest` plus `src/test/resources/schema-history/`); when a schema version is published, record it there.
- **The listener does not start with the context, and that is deliberate.** An unresolvable bootstrap address is a *configuration* error to the Kafka client, so constructing the consumer throws and used to kill the whole application — no alerts, no login, over a denormalised column. `VoteEventListenerStarter` starts it afterwards and retries every `nabat.kafka.listener-retry-interval`, which also means a broker that arrives later is picked up without a restart. Do not set `autoStartup` back to true.
- **Deployment order matters for a message-format change, and only nabat-voting can go first.** A broker outage is now survivable; a format mismatch is not, in either direction — this service reads what nabat-voting writes. So: merge and publish nabat-voting, then nabat-app. Two consequences worth knowing before you plan a release:
  - nabat-app's PR build cannot be green until the new voting image is published, because `VotingServiceIntegrationTest` runs the published one. Use `-Dnabat.voting.image=` locally in the meantime.
  - In the window between the two deploys, this service consumes messages it cannot read. `ErrorHandlingDeserializer` and the error handler turn them into logged, skipped records with their offsets committed — so they are not retried later. The counts for those alerts recover on the next vote, or by resetting the `nabat-app-group` offsets to earliest.
...
| Voting integration untested | ✅ Done | `VotingServiceIntegrationTest` starts nabat-voting, Kafka and a second Postgres with Testcontainers and votes through the real HTTP path. Select the image with `-Dnabat.voting.image`; the default is the published one, re-pulled if the local copy is over an hour old — without that, "the published image" quietly means whatever this machine last pulled. This is also the only test where `nabat.kafka.enabled` is on, so it is the only place the two repositories are checked to still agree on the vote topic. It found the Flyway defect below on its first run, and the stale-image trap on the day the topic changed. |
