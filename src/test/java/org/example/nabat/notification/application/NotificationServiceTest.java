package org.example.nabat.notification.application;
...
import org.example.nabat.notification.application.port.out.NotificationRepository;
...
class NotificationServiceTest {
    ...
    /**
     * The command now carries this module's {@link NotificationType} directly. Mapping a
     * vote onto one is the voting module's business — see {@code ExternalVoteService} —
     * so what is asserted here is the title and message this module derives from the
     * type it was given.
     */
    @ParameterizedTest
    @CsvSource({
            "ALERT_UPVOTED,Your alert was upvoted",
            "ALERT_DOWNVOTED,Your alert was downvoted",
            "ALERT_CONFIRMED,Your alert was confirmed"
    })
    void sendVoteNotification_persistsAndDeliversMappedNotification(
        ...
        Notification result = service.sendVoteNotification(
                new SendNotificationUseCase.VoteNotificationCommand(
                        user, voter, alert, "Title", expectedType));
